...
// Se han separado las importaciones para que apunten a sus archivos específicos
...
export function FractionalizeClient({ nft }: { nft: NftData }) {
    ...
    const handleFractionalize = async () => {
        ...
        tx.moveCall({
            ...
            arguments: [
                tx.object(nft.id),
                tx.pure.vector('u64', sharesToTransfer.map(s => (parseFloat(s.percentage) * 100).toString())), // Porcentajes como enteros x100
                tx.pure.vector('address', sharesToTransfer.map(s => s.recipient))
            ]
        });
    ...
    return (
        <div className="min-h-screen pt-24 pb-12 bg-background">
            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="mb-8"><Button asChild variant="outline" className="glass-card"><Link href="/"><ArrowLeft className="w-4 h-4 mr-2" /> Back</Link></Button></div>
                <div className="max-w-4xl mx-auto">
                    ...
                    <Card className="mb-8 glass-card flex items-center gap-6 p-4">
                        <img src={nft.imageUrl} alt={nft.name} className="w-24 h-24 object-cover rounded-lg" />
                        <div className="flex-grow">
                            <CardTitle className="text-foreground">{nft.name}</CardTitle>
                            <CardDescription>ID: {nft.id.slice(0, 6)}...{nft.id.slice(-4)}</CardDescription>
                        </div>
                    </Card>
                                ...
                                <Card key={share.id} className="glass-card p-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                                        <div>
                                            ...
                                            <Input id={`percentage-${index}`} type="number" value={share.percentage} onChange={(e: { target: { value: string; }; }) => handleShareChange(index, 'percentage', e.target.value)} placeholder="e.g., 25" className="mt-1"/>
                                        </div>
                                        <div>
                                            <Label htmlFor={`recipient-${index}`} className="text-muted-foreground">Recipient Address</Label>
                                            <div className="flex items-center gap-2">
                                                <Input id={`recipient-${index}`} value={share.recipient} onChange={(e: { target: { value: string; }; }) => handleShareChange(index, 'recipient', e.target.value)} placeholder="0x..." className={`mt-1 font-mono ${share.recipient && !isAddressValid ? 'border-destructive' : ''}`}/>
                                                {shares.length > 1 && (<Button variant="ghost" size="icon" onClick={() => handleRemoveShare(share.id)}><X className="w-4 h-4 text-destructive" /></Button>)}
                                            </div>
                                        </div>
                                    </div>
                                </Card>
                    ...
                    <Button variant="outline" onClick={handleAddShare} className="mt-4 w-full glass-card"><PlusCircle className="w-4 h-4 mr-2" /> Add Recipient</Button>

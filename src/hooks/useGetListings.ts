...
// --- INTERFACES BASADAS EN EL JSON REAL DE LA API ---
interface Attribute {
  fields: {
    key: string;
    value: string;
      ...
      try {
        ...
        const listingsWithDetails: NftListing[] = listingsData
          .map((node: any) => {
            ...
            console.log('[useGetListings] Verificando attributes:', fields.nft.fields.attributes);

            const contentTypeAttr = Array.isArray(fields.nft.fields.attributes) 
              ? fields.nft.fields.attributes.find(attr => {
                  // Este log nos dirá exactamente qué está viendo la función
            ...
            console.log(`[useGetListings] Content-Type encontrado: ${contentType}`);
        ...
        console.log('✅ [useGetListings] Listings finales procesados:', listingsWithDetails);

...
import { PurchaseReceiptCard } from '@/components/PurchaseReceiptCard'; // <-- Importa el nuevo componente
import { Loader2, Store, BadgeCheck, PackageOpen, Inbox, History, Edit, Star, BarChart2, Coins } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Skeleton } from '@/components/ui/skeleton';

// Interfaces
interface ProviderProfile { data: { objectId: string; content: { fields: { id: { id: string }, active_listings: string[], total_reviews: string, total_rating_points: string } } } }
...
function StatCard({ title, value, icon: Icon }: { title: string, value: string | number, icon: React.ElementType }) {
    return (
        <Card className="glass-card"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{title}</CardTitle><Icon className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold text-foreground">{value}</div></CardContent></Card>
    );
}
        ...
        <div className="space-y-8">
            ...
            <Tabs defaultValue="collection" className="w-full">
                ...
                <TabsContent value="collection" className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{nfts.map((nft) => ( <ListableNftCard key={nft.data.objectId} nft={nft.data as any} onActionSuccess={() => {}} /> ))}</div>
                ...
                <TabsContent value="memories" className="mt-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{poes.map((poe) => ( <ProofOfExperienceCard key={poe.data.objectId} poe={poe.data as any} /> ))}</div>
                ...
                <TabsContent value="reviews" className="mt-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{receipts.map((receipt) => ( <PurchaseReceiptCard key={receipt.data.objectId} receipt={receipt.data as any} /> ))}</div>
...
function ProviderDashboard({ providerProfile, nfts, poes, receipts }: { providerProfile: ProviderProfile, nfts: ExperienceNFT[], poes: ProofOfExperience[], receipts: PurchaseReceipt[] }) {
    ...
    const { data: activeListings, isLoading: isLoadingActiveListings } = useSuiClientQuery(
        'multiGetObjects', 
        { ids: providerProfileFields.active_listings, options: { showContent: true } },
        { enabled: providerProfileFields.active_listings.length > 0 }
    );
        ...
        <div className="space-y-8">
            ...
            <Tabs defaultValue="listings" className="w-full">
                ...
                <TabsContent value="listings" className="mt-6">
                    ...
                    {activeListings && activeListings.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{activeListings.map((listing) => (listing.data && <ListableNftCard key={listing.data.objectId} nft={listing.data as any} onActionSuccess={handleActionSuccess} isListing /> ))}</div>
                ...
                <TabsContent value="inventory" className="mt-6">
                    {nfts.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{nfts.map((nft) => (<ListableNftCard key={nft.data.objectId} nft={nft.data as any} onActionSuccess={handleActionSuccess} />))}</div>
                    ) : (<EmptyState icon={Inbox} title="Your inventory is empty" description="Create a new experience NFT to start selling." />)}
                </TabsContent>
                <TabsContent value="memories" className="mt-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{poes.map((poe) => ( <ProofOfExperienceCard key={poe.data.objectId} poe={poe.data as any} /> ))}</div>
                ...
                <TabsContent value="reviews" className="mt-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{receipts.map((receipt) => ( <PurchaseReceiptCard key={receipt.data.objectId} receipt={receipt.data as any} /> ))}</div>
                    {receipts.length === 0 && <EmptyState icon={Edit} title="No Pending Reviews" description="When a customer buys an NFT, their receipt will appear here until they leave a review." />}
                </TabsContent>
            </Tabs>
        </div>
    );
